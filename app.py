...
for column in df.columns: # fixing error with datatpe not benig float
...
df = (
    # pd.concat([df, pd.DataFrame([{"Year": MIN_YR - 1}])]], ignore_index=True) # maybe? im not sure lol
    pd.concat([df, pd.DataFrame([{"Year": MIN_YR - 1}])]) # idk why thsi no work
